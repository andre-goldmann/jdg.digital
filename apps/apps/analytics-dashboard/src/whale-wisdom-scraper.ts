/**
 * WhaleWisdom Scraper - TypeScript Version
...
interface WhaleWisdomError {
  error: string;
  filer_name: string;
  url: string;
}

export class WhaleWisdomScraper {
  ...
  async getFilerData(
    filerName: string
  ): Promise<WhaleWisdomData | WhaleWisdomError> {
  ...
  private _extractFundInfo($: cheerio.CheerioAPI): FundInfo {
    const info: FundInfo = {};
    ...
    const description = $('div')
      .filter((i, el) => {
        const text = $(el).text();
        return (
          text.includes('based out of') &&
          text.includes('13F filing') &&
          text.length < 1000
        );
      })
      .first()
    ...
    if (description) {
      ...
      const concentrationMatch = description.match(
  ...
  private _extractTopBuys($: cheerio.CheerioAPI): StockData[] {
    const buys: StockData[] = [];
  ...
  private _extractTopSells($: cheerio.CheerioAPI): StockData[] {
    const sells: StockData[] = [];
    ...
    $('table').each((i, table) => {
      ...
      if (hasStockData && hasPercentChange && i > 0) {
        // Skip first table (buys)
        $table.find('tr').each((j, row) => {
          const $row = $(row);
          const cells = $row.find('td');
  ...
  private _extract13FActivity($: cheerio.CheerioAPI): ActivityData {
    ...
    $('table').each((i, table) => {
      ...
      if (
        tableText.includes('Market Value') &&
        tableText.includes('New Purchases')
      ) {
        $table.find('tr').each((j, row) => {
          const $row = $(row);
          const cells = $row.find('td');
